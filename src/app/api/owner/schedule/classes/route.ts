...
export async function GET() {
  ...
  if (queryError) return NextResponse.json({ error: "Internal server error." }, { status: 500 });

  const hydrated = await withLookups((data ?? []) as ScheduleClassRow[]);
  return NextResponse.json({ classes: hydrated });
}
...
export async function POST(request: NextRequest) {
  ...
  const { data, error: insertError } = await supabaseAdmin
    .from("schedule_classes")
    .insert({ ...normalized.value, updated_at: new Date().toISOString() })
    .select(
      "id, track_id, name, class_time, duration_minutes, class_days, start_date, end_date, default_coach_user_id, size_limit, reservation_cutoff_hours, calendar_color, created_at, updated_at"
    )
    .single();
