...
async function getDashboardData(organizationId: string, members: GymMemberRow[]): Promise<DashboardData> {
  const now = new Date();
  ...
  const orgUserIds = Array.from(new Set(((membershipsResult.data ?? []) as any[]).map((row) => row.user_id).filter(Boolean)));

  const tasksResult = orgUserIds.length
    ? await supabaseAdmin
        .from("tasks")
        .select("id, title, due_date, priority, status, is_complete, member_id")
        .in("member_id", orgUserIds as string[])
        .gte("due_date", todayYmd)
        .lte("due_date", tomorrowYmd)
        .order("due_date", { ascending: true })
    : { data: [], error: null };

  const taskAssigneesResult = orgUserIds.length
    ? await supabaseAdmin
        .from("app_users")
        .select("id, full_name, email")
        .in("id", orgUserIds as string[])
    : { data: [], error: null };
...
export default async function GymDashboardPage({
  ...
  const dashboardData = await getDashboardData(organizationId, members);
