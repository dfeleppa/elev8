...
export default function MemberWorkoutClient() {
  ...
  const loadWeek = useCallback(
    async (dateStr: string) => {
      if (!organizationId || !trackId) return;
      const weekStart = startOfWeek(dateStr);
      setWeekLoading(true);
      try {
        const res = await fetch(
          `/api/programming/week?organizationId=${organizationId}&trackId=${trackId}&startDate=${weekStart}`,
          { cache: "no-store" }
        );
  ...
  useEffect(() => {
    ...
    (async () => {
      const res = await fetch(
        `/api/programming/results?organizationId=${organizationId}&startDate=${selectedDay}&endDate=${selectedDay}&memberId=${userId}`,
        { cache: "no-store" }
      );
      const payload = await res.json();
  ...
  }, [organizationId, userId, selectedDay]);
  ...
  useEffect(() => {
    ...
    for (const block of enabledBlocks) {
      (async () => {
        const res = await fetch(
          `/api/programming/leaderboards?organizationId=${organizationId}&blockId=${block.id}&dayDate=${selectedDay}`,
          { cache: "no-store" }
        );
        const payload = await res.json();
  ...
  }, [organizationId, selectedDay, dayCache]);
  ...
  function afterRecord(blockId: string) {
    ...
    if (leaderboards[blockId] !== undefined) {
      fetch(
        `/api/programming/leaderboards?organizationId=${organizationId}&blockId=${blockId}&dayDate=${selectedDay}`,
        { cache: "no-store" }
      )
        .then((r) => r.json())
...
function RecordModal({ block, organizationId, trackId, dayDate, userId, onClose, onSaved }: RecordModalProps) {
